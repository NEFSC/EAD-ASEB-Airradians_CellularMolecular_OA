...
BCA_stand_plots <- BCA_standards_means %>% # QUADRATIC SMOOTH LINE WORKS BEST HERE (MANUFACTURERS INSTRUCTIONS)
...
b1 <- 0.00127
c1 <- 0.0347
# EQ: (-(b1) + sqrt( (b1^2) - (4*(((a1)-Abs_562nm))*(c1)) ))/(2*((a1)-Abs_562nm))

# Standard curve, Plate 2
a2 <- -1.3*10^-7
b2 <- 0.00125
c2 <- 0.045
# EQ: (-(b2) + sqrt( (b2^2) - (4*a2*(c2-Abs_562nm)) ) ) / (2*a2)
...
TotalProtein_final <- raw_BCA_merged_om %>% 
                      ...
                      dplyr::mutate(TotalProtein_ug_mL = # now calculate ug mL from standard curve
                                    case_when(Plate == 1 ~ ((-(b1) + sqrt( (b1^2) - (4*a1*(c1-Abs_562nm)) ) ) / (2*a1)), # for plate 1
                                              Plate == 2 ~ ((-(b2) + sqrt( (b2^2) - (4*a2*(c2-Abs_562nm)) ) ) / (2*a2)) ), # for plate 2  
                                    TotalProtein_ug = TotalProtein_ug_mL*0.025) %>% # ug per mL concentration to ug in 25 ul sample 
...
ATP_final <- raw_ATP_merged_om %>% 
                      ...
                      dplyr::mutate(ATP_umol = # correct the raw abs, subtract background
                                    case_when(Run_date == 20230828 ~ ((Abs_570nm + 0.0114)/0.0385)*1000, # for plate 1 - nmol to umol
                                              Run_date == 20230829 ~ ((Abs_570nm + 0.0249)/0.0994)*1000 ) )  %>% # for plate 2 - nmol to umol
                      dplyr::group_by(Day,pCO2_history,pCO2_exposure,
                                      Unique_ID, Run_date, Scallop_ID) %>% # group by to get the means
                      dplyr::summarise(mean_ATP_umol = mean(ATP_umol),
                                       sd_ATP_umol   = sd(ATP_umol),
                                       n = n(),
                                       se_ATP_umol   = sd_ATP_umol / sqrt(n))
...
MASTER_DF <- merge(ATP_final_prepmerge, 
                ...
                dplyr::mutate(ATP_umol_ug_protein = (mean_ATP_umol/2)/mean_TotalProtein_ug)
...
Day1_ATP_RxnNorm <- MASTER_DF %>% # data %>%
  ...
  dplyr::mutate(ATP_umol_ug_protein = as.numeric(ATP_umol_ug_protein)) %>% 
  dplyr::select(c('Day','pCO2_exposure','pCO2_history','ATP_umol_ug_protein')) %>% 
  na.omit() %>%   
  ...
  dplyr::summarise( # summarise to acquire the mean and SE for plotting
    ATP_mean = mean(ATP_umol_ug_protein), # mean
    ATP_sd = sd(ATP_umol_ug_protein), # sd
    ATP_n = n(), # count
  ...
  ggplot(aes(x=pCO2_exposure, y=ATP_mean, group=pCO2_history)) + # call the new mean
    ...
    ggtitle("ATP, Day 1") +
    scale_y_continuous(expand = c(0, 0), limits = c(0, 40)) + # true 0 origin
...
Day1_TotalProtein_RxnNorm <- TotalProtein_final %>% # data %>%
  ...
  ggplot(aes(x=pCO2_exposure, y=TP_mean, group=pCO2_history)) + # call the new mean
    ...
    theme_classic() +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position = "none") + 
    scale_y_continuous(expand = c(0, 0), limits = c(0, 150)) + # true 0 origin
