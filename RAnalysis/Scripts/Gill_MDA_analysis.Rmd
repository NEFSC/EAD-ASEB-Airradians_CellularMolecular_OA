...
# raw_MDA_merged_om %>% dplyr::filter((Day == 14 & pCO2_history == 'severe' & pCO2_exposure == 'moderate'))
```
...
# plot the data to see any glaring outliers 
MDA_plot <- MASTER_DF %>% 
                      ggplot(aes(y = mean_MDA_µmol, 
                                 x  = Scallop_ID)) +
                      theme_bw() +
                      ggtitle("mean MDA") +
                      geom_point()
TP_plot <- MASTER_DF %>% 
                      ggplot(aes(y = mean_TotalProtein_ug, 
                                 x  = Scallop_ID)) +
                      theme_bw() +
                      ggtitle("mean TP") +
                      geom_point()
MDAcor_plot <- MASTER_DF %>% 
                      ggplot(aes(y = MDA_µmol_µg_protein, 
                                 x  = Scallop_ID)) +
                      theme_bw() +
                      ggtitle("mean MDA_per_protein") +
                      geom_point()
ggarrange(MDA_plot,
          TP_plot,
          MDAcor_plot,
...
MASTER_DF %>% dplyr::filter(MDA_µmol_µg_protein > 7.5)
...
Day1_MDA_RxnNorm <- MDA_master %>% # data %>%
  dplyr::filter(!MDA_µmol_µg_protein > 7.5) %>% # day 1 outlier
...
Day14_MDA_RxnNorm <- MDA_master %>% # data %>%
...
Day1_MDA_RxnNorm <- MASTER_DF %>% # data %>%
  dplyr::filter(!MDA_µmol_µg_protein > 7.5) %>% 
  dplyr::filter(Day == 1) %>% # filter for desired date of rhte experiment
...
ModeratepCO2_MatchMismatch_RxnNorm <- MASTER_DF %>% # data %>%
  ...
  ggplot(aes(x=as.factor(Day), y=MDA_mean, group=Match_Mismatch)) + # call the new mean
    ...
    # scale_y_continuous(expand = c(0, 0), limits = c(10, 40)) + # true 0 origin
    # scale_y_continuous(expand = c(0, 0), limits = c(0, 2.5)) + # true 0 origin
    labs(y= "MDA (µmol ng protein-1)", x = "Time exposed (days)")
...
# for loop m
Colorimetric_assays          <- as.data.frame(list(c('MDA_µmol_µg_protein',
                                                'mean_TotalProtein_ug')))

# prepare data for analysis
data <- MASTER_DF %>% 
  dplyr::select(c("Day","pCO2_history","pCO2_exposure",
                  "MDA_µmol_µg_protein","mean_TotalProtein_ug")) 
