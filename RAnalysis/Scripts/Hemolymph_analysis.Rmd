...
D1_PropAlive_Twoway <- (glm(SYBR_PI_prop_alive_ADJ ~ 
                            ...
                            data = Day1_datafilt))
shapiro.test(resid(aov(D1_PropAlive_Twoway))) # 0.7753
leveneTest(D1_PropAlive_Twoway) # 0.6848
...
shapiro.test(resid((D1_PropAlive_Mod))) # 0.8239
leveneTest(D1_PropAlive_Mod) # 0.9854
...
leveneTest(D1_PropAlive_Sev) # 0.1078
...
D14_PropAlive_Twoway <- (glm(SYBR_PI_prop_alive_ADJ ~ 
                             ...
                             family = binomial(link = 'logit'),
                            data = Day14_datafilt))
shapiro.test(resid(aov(D14_PropAlive_Twoway))) # 0.3293
leveneTest(D14_PropAlive_Twoway) # 0.06582  *
...
```{r Two way ANOVAs - Mitochondrial membrane potential}
...
## REACTION NORM STATISTICS

## BASELINE: One way ANOVA pCO2 history effects (UNDER MATCHED EXPOSURE ONLY)
```{r Rxn Norm stats: Baseline}
...
# prepare data for analysis - mutate new column with matched exposure to history, filter for baseline exposures
data2 <- data_filt %>% 
                      dplyr::select(c("Date","pCO2_history","pCO2_exposure",
                                      "SYBR_PI_prop_alive_ADJ",
                                      "MitoSoxGreen_Mean_FL1", "JC10_FL2_FL1_Ratio")) %>% 
                      dplyr::mutate(All_treatment = paste0(pCO2_history, "_",pCO2_exposure)) %>% 
                      dplyr::filter(All_treatment %in% c('low_low',
                                                         'moderate_moderate',
                                                         'severe_severe'))

# Call the cumulative dataframe that we will write to in the for loop below
baseline_stats        <- data.frame() # start dataframe 
stats.table           <- data.frame(matrix(nrow = 1, ncol = 9)) # create dataframe to save cumunalitively during for loop
...
for (d in 1:nrow(Dates)) {
  ...
  #     dplyr::filter(pCO2_history %in% pCO2history_treatments[i,1])
      ...
      for (m in 1:nrow(FlowCy_probes)) {
        ...
        colNum   <- which( colnames(data_by_date)==FlowCy_probes[m,])
        data_by_date[,colNum] = as.numeric(data_by_date[,colNum])
        
        # :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
        # beta regression - this is one option ::::::::::::::::::::::
        # Note: if your data includes exact zeros and ones, GLM logi link is more appropriate, 
        # whereas if data is strickly between (continuous data) 0 and 1 beta gression is the best choice
        # USE BETA REGRESSION FOR THESE DATA!
        # comment out below if you devide upon another method...
        
        modBetaReg     <- betareg(data_by_date[,colNum]~pCO2_exposure, data = data_by_date) # beta reg model suitable for prop data
        ...
        modSUMMARYchi  <- signif(modSUMMARY[["Chisq"]][1], 4) # cal p value for anova table - 2 sig figs
        modSUMMARYPval <- signif(modSUMMARY[["p.value"]][1], 4) # cal p value for anova table - 2 sig figs
        testRun        <- "Beta regression model"
        
        # ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
        # glm ith a binomial logit link function for data between 0-1 ::::::::::::::::::
        # Note: if your data includes exact zeros and ones, GLM logi link is more appropriate, 
        # whereas if data is strickly between (continuous data) 0 and 1 beta gression is the best choice
        # comment out below if you devide upon another method...
        
        # modGLMlogit    <- (glm(data_by_date[,colNum] ~ pCO2_exposure, family = binomial(link = 'logit'), data = data_by_date))
        # modSUMMARY     <- summary(aov(modGLMlogit)) # create anova table summary
        ...
        # testRun        <- "GLM logit link"
        
        # :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
        # One way anova of log transformed data :::::::::::::::::::::::::::::::::::::::::
        # Note: another option for continuous data between 0-1 is to log trans ANOVA 
        # comment out below if you devide upon another method...
        
        # modAOV   <- aov(lm(log(data_by_history[,colNum])~pCO2_exposure, data = data_by_history)) # log transformed!
        ...
        # modSUMMARYchi  <- NA  
        # modSUMMARYPval <- signif(modSUMMARY[[1]][["Pr(>F)"]][1], 4) # cal p value for anova table - 2 sig figs
        # testRun        <- "One-way ANOVA (log trans)"   

        # regardless of test, just output NA for Shapiro and levenes (each aside frm anova are non-parametric!)
        ...
        colNum   <- which( colnames(data_by_date)==FlowCy_probes[m,])
        data_by_date[,colNum] = as.numeric(data_by_date[,colNum])
        modAOV   <- aov(lm(data_by_date[,colNum]~pCO2_exposure, data = data_by_date))
        shapTEST <- shapiro.test(resid(modAOV))
        levTEST  <- leveneTest(modAOV)
            ...
            if(shapTEST$p.value> 0.05 & levTEST$`Pr(>F)`[1] > 0.05) { # if normal - run anova
              ...
              modSUMMARY     <- kruskal.test(data_by_date[,colNum] ~ pCO2_exposure, data = data_by_date) 
        ...
        stats.table$Date          <- data_by_date$Date[1]
        ...
        baseline_stats <- rbind(baseline_stats,df) #bind to a cumulative list dataframe
        print(baseline_stats) # print to monitor progress      
...
View(baseline_stats %>% dplyr::arrange(Pvalue))
baseline_stats

write.csv(baseline_stats, "C:/Users/samjg/Documents/Github_repositories/Airradians_CellularMolecular_OA/RAnalysis/Output/FlowCytometry/RxnNormStats_Baseline.csv")



## posthoc effects for significnat effets, print sig effects
baseline_stats %>% dplyr::filter(Pvalue < 0.05)



# 24 hours mit mem potential one way anova
ANOVA_24h_mitmem <- aov(lm(JC10_FL2_FL1_Ratio ~ pCO2_history, 
                              data = (data2 %>% filter(Date %in% 20230502))))
summary(ANOVA_24h_mitmem) # 0.0174    
ANOVA_24h_mitmem.post_hoc <- emmeans(ANOVA_24h_mitmem, specs = pairwise ~ pCO2_history)
ANOVA_24h_mitmem.post_hoc
 # contrast          estimate   SE df t.ratio p.value
 # low - moderate      -29.46 8.85 12  -3.330  0.0153 # low and moderate differ
 # low - severe        -20.02 8.85 12  -2.264  0.1003
 # moderate - severe     9.44 8.85 12   1.067  0.5514


# 14 days mitsox
KW_14d_mitosox <- kruskal.test(MitoSoxGreen_Mean_FL1 ~ pCO2_history, 
                              data = (data2 %>% filter(Date %in% 20230516)))
KW_14d_mitosox # 0.04264    
rstatix::dunn_test(MitoSoxGreen_Mean_FL1 ~ pCO2_history, data = (data2 %>% filter(Date %in% 20230516)))
# #                                         N1 N2 STATISTIC  p           pADJ
# 1	MitoSoxGreen_Mean_FL1	low	moderate	    4	5	-1.5119790	0.13053920	0.26107839	ns
# 2	MitoSoxGreen_Mean_FL1	low	severe	      4	4	0.9078413	0.36396208	0.36396208	ns
# 3	MitoSoxGreen_Mean_FL1	moderate	severe	5	4	2.4689278	0.01355186	0.04065557	* # moderate and high differ
\
?dunn_test
?kruskal.test
?betareg
...
## EXPOSURE: One way ANOVA pCO2 exposure effects WITHIN HISTORY
*  Effect of pCO2 exposure within each pCO2 history and for each dependent variable
*  loop one way (kruskal wallis non param) and beta reg (survival data) 
```{r Rxn Norm stats: Exposure}
...
# Call the cumulative dataframe that we will write to in the for loop below
exposure_stats        <- data.frame() # start dataframe 
  ...
  for (i in 1:nrow(pCO2history_treatments)) {
    ...
    data_by_history <- data_by_date %>% 
      ...
      for (m in 1:nrow(FlowCy_probes)) {
        ...
        df             <- data.frame(stats.table) # name dataframe for this single row
        exposure_stats <- rbind(exposure_stats,df) #bind to a cumulative list dataframe
        print(exposure_stats) # print to monitor progress      
...
View(exposure_stats %>% dplyr::arrange(Pvalue))
exposure_stats


write.csv(exposure_stats, "C:/Users/samjg/Documents/Github_repositories/Airradians_CellularMolecular_OA/RAnalysis/Output/FlowCytometry/RxnNormStats_Exposure.csv")



## posthoc effects for significnat effets, print sig effects
exposure_stats %>% dplyr::filter(Pvalue < 0.05)



# 24 hours severe proportion alive beta regression 
betareg_24hr_high <- betareg(SYBR_PI_prop_alive_ADJ~pCO2_exposure,
                              data = data2 %>% filter(Date %in% 20230502 & pCO2_history %in% 'severe'))
joint_tests(betareg_24hr_high) # 0.0109 - yes significant 
betareg_24hr_high.post_hoc <- emmeans(betareg_24hr_high, specs = pairwise ~ pCO2_exposure)
betareg_24hr_high.post_hoc # mod different from severe betareg_24hr_high.post_hoc
 # contrast          estimate     SE  df z.ratio p.value
 # low - moderate     -0.0530 0.0495 Inf  -1.071  0.5318
 # low - severe        0.0929 0.0531 Inf   1.751  0.1865
 # moderate - severe   0.1459 0.0486 Inf   3.001  0.0076


# 14 days low history, proportion alive beta regression
betareg_14d_low <- betareg(SYBR_PI_prop_alive_ADJ~pCO2_exposure,
                             data = data2 %>% filter(Date %in% 20230516 & pCO2_history %in% 'low'))
joint_tests(betareg_14d_low) # 0.0291 - yes significant 
betareg_14d_low.post_hoc <- emmeans(betareg_14d_low, specs = pairwise ~ pCO2_exposure)
betareg_14d_low.post_hoc # mod different from severe betareg_24hr_high.post_hoc
# contrast          estimate     SE  df z.ratio p.value
#  low - moderate      0.1622 0.0634 Inf   2.560  0.0282
#  low - severe        0.1009 0.0600 Inf   1.682  0.2122
#  moderate - severe  -0.0613 0.0677 Inf  -0.904  0.6376
Low_d14_df <- (data2 %>% filter(Date %in% 20230516 & pCO2_history %in% 'low')) %>% na.omit()
Rmisc::summarySE(Low_d14_df, groupvars = 'pCO2_exposure', measurevar = 'SYBR_PI_prop_alive_ADJ')
#           N mean      sd          se
# low	      4	0.8890386	0.03187177	0.01593589	0.0507151
# moderate	5	0.7045212	0.14199022	0.06349996	0.1763041
# severe	  5	0.7566827	0.14220507	0.06359604	0.1765709


# 14 days low history, mitosox green kruskal wallis
KW_14d_low_mitosox <- kruskal.test(MitoSoxGreen_Mean_FL1 ~ pCO2_exposure, 
                                  data = data2 %>% filter(Date %in% 20230516 & pCO2_history %in% 'low'))
KW_14d_low_mitosox # 0.03175
rstatix::dunn_test(MitoSoxGreen_Mean_FL1 ~ pCO2_exposure, data = data2 %>% filter(Date %in% 20230516 & pCO2_history %in% 'low'))
#                                         N1 N2 STATISTIC  p           pADJ
# 1	MitoSoxGreen_Mean_FL1	low	moderate	    4	5	-2.3697163	0.01780174	0.05340521	ns # though sig, marginal posthoc
# 2	MitoSoxGreen_Mean_FL1	low	severe	      4	5	-0.3741657	0.70828101	0.70828101	ns 
# 3	MitoSoxGreen_Mean_FL1	moderate	severe	5	5	2.1166010	0.03429372	0.06858744	ns   # though sig, marginal posthoc

# 14 days moderate history, mitosox green kruskal wallis
KW_14d_mod_mitosox <- kruskal.test(MitoSoxGreen_Mean_FL1 ~ pCO2_exposure, 
                                  data = data2 %>% filter(Date %in% 20230516 & pCO2_history %in% 'moderate'))
KW_14d_mod_mitosox # 0.01072
rstatix::dunn_test(MitoSoxGreen_Mean_FL1 ~ pCO2_exposure, data = data2 %>% filter(Date %in% 20230516 & pCO2_history %in% 'moderate'))
#                                          N1 N2 STATISTIC  p           pADJ
# 1	MitoSoxGreen_Mean_FL1	low	moderate	    5	5	-1.814229	0.069642405	0.139284810	ns
# 2	MitoSoxGreen_Mean_FL1	low	severe      	5	4	 1.265037	0.205858228	0.205858228	ns
# 3	MitoSoxGreen_Mean_FL1	moderate	severe	5	4	 2.975508	0.002925033	0.008775098	** # moderate diff from severe


# 14 days low history, Jc10 one way anova
ANOVA_14d_low_mitmem <- aov(lm(JC10_FL2_FL1_Ratio ~ pCO2_exposure, 
                                  data = data2 %>% filter(Date %in% 20230516 & pCO2_history %in% 'low')))
summary(ANOVA_14d_low_mitmem) # 0.0149   
ANOVA_14d_low_mitmem.post_hoc <- emmeans(ANOVA_14d_low_mitmem, specs = pairwise ~ pCO2_exposure)
ANOVA_14d_low_mitmem.post_hoc
 # contrast          estimate     SE df t.ratio p.value
 # low - moderate      -0.295 0.0837 11  -3.527  0.0121 # low and moderate differ
 # low - severe        -0.134 0.0837 11  -1.603  0.2851
 # moderate - severe    0.161 0.0789 11   2.041  0.1484
