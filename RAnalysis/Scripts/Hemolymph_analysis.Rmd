...
# PERCENT/PROPORTION LIVE AND DEAD CELLS 
    ...
    dplyr::mutate(SYBR_PI_percent_alive=
            ifelse(Scallop_ID %in% c('51', '59', '60','20'), 
                   NA, as.numeric(sub("%", "", SYBR_PI_percent_alive)))) %>% # for #s insert Na, else insert oG data
    dplyr::mutate(SYBR_PI_count_alive=
            ifelse(Scallop_ID %in% c('51', '59', '60','20'), 
                   NA, as.numeric(sub(",", "", SYBR_PI_count_alive)))) %>% # for #s insert Na, else insert oG data

    dplyr::mutate(SYBR_PI_percent_dead=
            ifelse(Scallop_ID %in% c('51', '59', '60','20'), 
                   NA, as.numeric(sub("%", "", SYBR_PI_percent_dead)))) %>% # for #s insert Na, else insert oG data
    dplyr::mutate(SYBR_PI_count_dead=
            ifelse(Scallop_ID %in% c('51', '59', '60','20'), 
                   NA, as.numeric(sub(",", "", SYBR_PI_count_dead)))) %>% # for #s insert Na, else insert oG data  
  
# MITOCHONDRIAL SUPEROXIDE DISMUTASE 
    # NA for mitosox greend data = #s 24 and 74
    dplyr::mutate(MitoSoxGreen_Mean_FL1=
            ifelse(Scallop_ID %in% c('24', '74'), NA, as.numeric(sub(",", "", MitoSoxGreen_Mean_FL1))))  %>% # for #s insert Na, else insert oG data
...
df_total
  ...
  low <- data_filt %>% 
     ...
     dplyr::mutate(PropLive_Transormed = log( (SYBR_PI_prop_alive_ADJ*100)  /(100-(SYBR_PI_prop_alive_ADJ*100))))

summary(aov(lm(PropLive_Transormed~pCO2_exposure, data = low)))
...
#  pCO2_history                 2 Inf   4.525  0.0108
#  pCO2_exposure                2 Inf   0.978  0.3760
#  pCO2_history:pCO2_exposure   4 Inf   0.744  0.5617
Anova(Day1_PropLive_betareg) # using Anova fro 'car' package
#                            Df  Chisq Pr(>Chisq)  
# pCO2_history                2 7.8064    0.02018 *
# pCO2_exposure               2 1.9720    0.37306  
# pCO2_history:pCO2_exposure  4 2.2002    0.69899 
...
#  pCO2_exposure                2 Inf   0.978  0.3760
#  pCO2_history:pCO2_exposure   4 Inf   0.744  0.5617
Anova(Day1_PropDead_betareg) # using Anova fro 'car' package
#                            Df  Chisq Pr(>Chisq)  
# pCO2_history                2 7.8064    0.02018 *
# pCO2_exposure               2 1.9720    0.37306  
# pCO2_history:pCO2_exposure  4 2.2002    0.69899 
...
joint_tests(Day14_PropLive_betareg) # using joint_tests from 'emmeans' package
 # model term                 df1 df2 F.ratio p.value
 # pCO2_history                 2 Inf   0.111  0.8947
 # pCO2_exposure                2 Inf   0.346  0.7074
 # pCO2_history:pCO2_exposure   4 Inf   0.881  0.4743
Anova(Day14_PropLive_betareg) # using Anova fro 'car' package
#                            Df  Chisq Pr(>Chisq)
# pCO2_history                2 0.1079     0.9475
# pCO2_exposure               2 0.5486     0.7601
# pCO2_history:pCO2_exposure  4 3.5972     0.4633
...
# pCO2_history                2   31.2   15.58   0.327  0.723
# pCO2_exposure               2   73.1   36.57   0.769  0.472
# pCO2_history:pCO2_exposure  4  402.6  100.65   2.116  0.100
# Residuals                  34 1617.5   47.57
...
#                             Df   Sum Sq Mean Sq F value   Pr(>F)    
# pCO2_history                2 10029463 5014732   9.196 0.000642 ***
# pCO2_exposure               2  8541876 4270938   7.832 0.001594 ** 
# pCO2_history:pCO2_exposure  4  6656962 1664241   3.052 0.029839 *  
# Residuals                  34 18541738  54534
TukeyHSD(Day14_MitoSox_AOV)
# $`pCO2_history:pCO2_exposure`
                                        # diff         lwr       upr     p adj
# moderate:low-low:low               -107.5367 -1892.06069 1676.9874 0.9999999
# severe:low-low:low                  510.2793 -1274.24469 2294.8034 0.9882426
# low:moderate-low:low              -1514.1447 -3298.66869  270.3794 0.1503322
# moderate:moderate-low:low         -1241.3527 -3025.87669  543.1714 0.3690146
# severe:moderate-low:low             434.5313 -1349.99269 2219.0554 0.9959307
# low:severe-low:low                 -457.2687 -2241.79269 1327.2554 0.9942615
# moderate:severe-low:low             764.4173 -1020.10669 2548.9414 0.8832710
# severe:severe-low:low               370.0733 -1414.45069 2154.5974 0.9986690
# severe:low-moderate:low             617.8160  -927.62714 2163.2591 0.9173324
# low:moderate-moderate:low         -1406.6080 -2952.05114  138.8351 0.0981349
# moderate:moderate-moderate:low    -1133.8160 -2679.25914  411.6271 0.3022141
# severe:moderate-moderate:low        542.0680 -1003.37514 2087.5111 0.9594117
# low:severe-moderate:low            -349.7320 -1895.17514 1195.7111 0.9975409
# moderate:severe-moderate:low        871.9540  -673.48914 2417.3971 0.6395142
# severe:severe-moderate:low          477.6100 -1067.83314 2023.0531 0.9808578
# low:moderate-severe:low           -2024.4240 -3569.86714 -478.9809 0.0034793 * sig here!
# moderate:moderate-severe:low      -1751.6320 -3297.07514 -206.1889 0.01673243 * sig here!
# severe:moderate-severe:low          -75.7480 -1621.19114 1469.6951 1.0000000
# low:severe-severe:low              -967.5480 -2512.99114  577.8951 0.5078496
# moderate:severe-severe:low          254.1380 -1291.30514 1799.5811 0.9997540
# severe:severe-severe:low           -140.2060 -1685.64914 1405.2371 0.9999974
# moderate:moderate-low:moderate      272.7920 -1272.65114 1818.2351 0.9995849
# severe:moderate-low:moderate       1948.6760   403.23286 3494.1191 0.00544003 * sig here!
# low:severe-low:moderate            1056.8760  -488.56714 2602.3191 0.3911439
# moderate:severe-low:moderate       2278.5620   733.11886 3824.0051 0.00074423 * sig here!
# severe:severe-low:moderate         1884.2180   338.77486 3429.6611 0.00790973 * sig here!
# severe:moderate-moderate:moderate  1675.8840   130.44086 3221.3271 0.02529673 * sig here!
# low:severe-moderate:moderate        784.0840  -761.35914 2329.5271 0.7546146
# moderate:severe-moderate:moderate  2005.7700   460.32686 3551.2131 0.00388663 * sig here!
# severe:severe-moderate:moderate    1611.4260    65.98286 3156.8691 0.03560733 * sig here!
# low:severe-severe:moderate         -891.8000 -2437.24314  653.6431 0.6122500
# moderate:severe-severe:moderate     329.8860 -1215.55714 1875.3291 0.9983667
# severe:severe-severe:moderate       -64.4580 -1609.90114 1480.9851 1.0000000
# moderate:severe-low:severe         1221.6860  -323.75714 2767.1291 0.2175130
# severe:severe-low:severe            827.3420  -718.10114 2372.7851 0.6994702
# severe:severe-moderate:severe      -394.3440 -1939.78714 1151.0991 0.9944207
...
#  low          moderate        3321 330 34     2650     3992  a    
#  moderate     moderate        3594 330 34     2923     4265  a    
#  low          severe          4378 330 34     3707     5049  ab   
#  moderate     low             4728 330 34     4056     5399  ab   
#  low          low             4835 426 34     3969     5702  ab   
#  severe       severe          5205 330 34     4534     5876   b   
#  severe       moderate        5270 330 34     4599     5941   b   
#  severe       low             5345 330 34     4674     6017   b   
#  moderate     severe          5600 330 34     4928     6271   b
