...
Challenge_ModuleMembership  <- read.csv(file="Output/Transcriptomics/WGCNA/challenge/Challenge_SignificantModules_Annotated.csv", 
                                        sep=',', header=TRUE) %>% 
                                        dplyr::select(!X) %>% 
...
length(modulePinkMagenta_Airradians.IDs) # 1041 $ 763
...
nrow(vstExp_ChallengeUP.long) # 28107 # 20358

vstExp_ChallengeUP.long.merge        <- merge(vstExp_ChallengeUP.long, Challenge_ModuleMembership, 
...
nrow(vstExp_ChallengeUP.long.meta.means) # 6246 # 4524
length(unique(vstExp_ChallengeUP.long.meta.means$Airradians.TranscriptID)) # 1041 # 754
```
...
length(modulesBrownRed_Airradians.IDs) # 1822 # 1251
#vst 
...
nrow(vstExp_ChallengeDOWN.long) # 49194 # 33615

vstExp_ChallengeDOWN.long.merge        <- merge(vstExp_ChallengeDOWN.long, Challenge_ModuleMembership, 
                                                by = 'Airradians.TranscriptID')
...
nrow(vstExp_ChallengeDOWN.long.meta.means) # 10932 # 7470
length(unique(vstExp_ChallengeDOWN.long.meta.means$Airradians.TranscriptID)) #1822 # 1245
```
...
Frontloaded_AirradiansTranscriptIDs <- merge((df_total_1.1 %>% 
                                             ...
                                             Challenge_ModuleMembership, 
                                             by = c('Protein_name','Airradians.TranscriptID'))
nrow(Frontloaded_AirradiansTranscriptIDs) # 565

# write csv of dfrontloaded genes
write.csv(Frontloaded_AirradiansTranscriptIDs, 
          file = "Output/Transcriptomics/WGCNA/challenge/Frontloaded_Activated/UPmodulePinkMagenta_Frontloaded.csv")
...
# Out put the plots 
pdf("Output/Transcriptomics/WGCNA/challenge/Frontloaded_Activated/UPmodulePinkMagenta_Frontloading.pdf", height=4, width =8)
...
Activated_AirradiansTranscriptIDs <- merge((df_total_1.1 %>% filter(Frontloaded_criteria == 'activated')),
                                             Challenge_ModuleMembership, 
                                             by = c('Protein_name','Airradians.TranscriptID'))
nrow(Activated_AirradiansTranscriptIDs) # 133


# write csv of dfrontloaded genes
write.csv(Activated_AirradiansTranscriptIDs, 
          file = "Output/Transcriptomics/WGCNA/challenge/Frontloaded_Activated/UPmodulePinkMagenta_Activated.csv")
...
leveneTest(Activated_AOV) #0.9981
Activated_SRH <- scheirerRayHare(mean.vstExp~pCO2_history*MatchMismatch,data=Activated_vst_levels)
#                             Df   Sum Sq      H p.value
# pCO2_history                 1   191900 8.1211 0.00438
# MatchMismatch                1      152 0.0064 0.93616
# pCO2_history:MatchMismatch   1    23264 0.9845 0.32109
# Residuals                  528 12332038
Activated_modKW <- kruskal.test(mean.vstExp~IDvar,data=Activated_vst_levels)
# Kruskal-Wallis chi-squared = 9.1121, df = 3, p-value = 0.02784
Activated_modDunn <-  dunnTest(mean.vstExp ~ IDvar,
              data=Activated_vst_levels,
              method="bh")
# lowbaseline - lowchange	        0.6449713	0.518945771	0.51894577	
# lowbaseline - severebaseline	 -1.3134759	0.189022630	0.28353395	
# lowchange - severebaseline	   -1.9584472	0.050177556	0.10035511	
# lowbaseline - severechange	   -2.0717261	0.038290995	0.11487299	
# lowchange - severechange	     -2.7166974	0.006593687	0.03956212	*
# severebaseline - severechange	 -0.7582501	0.448301248	0.53796150	
...
pdf("Output/Transcriptomics/WGCNA/challenge/Frontloaded_Activated/UPmodulePinkMagenta_FrontloadingAcivated.pdf", height=8, width =8)
ggarrange(Frontloaded_plot_1.1, Frontloaded_MEAN.SE_1.1,
...
CostlyActivation_AirradiansTranscriptIDs <- merge((df_total_1.2 %>% 
                                             ...
                                             Challenge_ModuleMembership, 
                                             by = c('Protein_name','Airradians.TranscriptID'))
nrow(CostlyActivation_AirradiansTranscriptIDs) # 866 # 584
# View(Frontloaded_AirradiansTranscriptIDs)
# write csv of dfrontloaded genes
write.csv(CostlyActivation_AirradiansTranscriptIDs, 
          file = "Output/Transcriptomics/WGCNA/challenge/Frontloaded_Activated/DOWNmoduleBrownRed_CostlyActivation.csv")
# truncate the vst Exp means (mean exp for each gene within pCO2 history * pCO2 exposure, N = 5 each)
...
# pCO2_history                  1    6.6   6.552  19.365 1.13e-05 ***
# MatchMismatch                 1    1.0   0.965   2.852   0.0914 .  
# pCO2_history:MatchMismatch    1    1.7   1.684   4.977   0.0258 *  
# Residuals                  2332  789.0   0.338 
mod <- scheirerRayHare(mean.vstExp~pCO2_history*MatchMismatch,data=CostlyActivation_vst_levels)
#                              Df     Sum Sq      H   p.value
# pCO2_history                  1   17562021 38.603 0.0000000
# MatchMismatch                 1    2884044  6.339 0.0118082
# pCO2_history:MatchMismatch    1    5549505 12.198 0.0004783
# Residuals                  2332 1036279988 
...
modDunn <-  dunnTest(mean.vstExp ~ IDvar,
              ...
              method="bh")
# lowbaseline - lowchange	       0.6892868	4.906428e-01	4.906428e-01	
# lowbaseline - severebaseline	 6.8630221	6.741880e-12	4.045128e-11 *	
# lowchange - severebaseline	   6.1737352	6.669520e-10	2.000856e-09 *
# lowbaseline - severechange	   2.6129910	8.975365e-03	1.346305e-02 *
# lowchange - severechange	     1.9237042	5.439167e-02	6.527001e-02 	
# severebaseline - severechange	-4.2500310	2.137409e-05	4.274818e-05 *
...
CostlyActivation_MEAN.SE_1.2 <- CostlyActivation_plotting %>% 
        dplyr::filter(MatchMismatch %in% c('baseline', 'change')) %>% 
             ggplot(aes(x=MatchMismatch, y = mean_meanvstExp, fill=pCO2_history)) +
               ...
               labs(y= "vst expression", 
                    x = "pCO2 exposure",
                    title = "Costly activation genes (N=584)") + 
               geom_errorbar(aes(ymin=(mean_meanvstExp)-(se_vstExp), # new means and se by treatment
                                 ymax=(mean_meanvstExp)+(se_vstExp)), # new means and se by treatment
                                 width=0,position=position_dodge(.4)) # width determines the length of the end ticks
...
# Out put the plots 
pdf("Output/Transcriptomics/WGCNA/challenge/Frontloaded_Activated/DOWNmoduleBrownRed_CostlyActivation.pdf", height=4, width =8)
