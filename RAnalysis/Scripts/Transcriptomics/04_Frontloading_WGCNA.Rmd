...
modulePinkBrown_Airradians.IDs       <- (Challenge_ModuleMembership %>% dplyr::filter(moduleColor %in%
                                                                                       c('pink')))$Airradians.TranscriptID

length(modulePinkBrown_Airradians.IDs) # 530
#vst 
vstExp_ChallengeUP.long              <- vstExp_Challenge %>% 
                                            dplyr::filter(Airradians.TranscriptID %in% modulePinkBrown_Airradians.IDs) %>% 
                                            pivot_longer(
                                              cols = !c(Airradians.TranscriptID), 
                                              names_to = "Sample_num",
                                              values_to = "vstExp"
                                            )
nrow(vstExp_ChallengeUP.long) # 14310
...
nrow(vstExp_ChallengeUP.long.meta.means) # 3180
length(unique(vstExp_ChallengeUP.long.meta.means$Airradians.TranscriptID)) # 530
```
...
modulesBrown_Airradians.IDs      <- (Challenge_ModuleMembership %>% dplyr::filter(moduleColor %in%
                                                                                        c('brown')))$Airradians.TranscriptID

length(modulesBrown_Airradians.IDs) # 1203
#vst 
vstExp_ChallengeDOWN.long              <- vstExp_Challenge %>% 
                                            dplyr::filter(Airradians.TranscriptID %in% modulesBrown_Airradians.IDs) %>% 
                                            pivot_longer(
                                              cols = !c(Airradians.TranscriptID), 
                                              names_to = "Sample_num",
                                              values_to = "vstExp"
                                            )
nrow(vstExp_ChallengeDOWN.long) # 32481
...
nrow(vstExp_ChallengeDOWN.long.meta.means) # 7218
length(unique(vstExp_ChallengeDOWN.long.meta.means$Airradians.TranscriptID)) #1203
```
...
write.csv(Frontloaded_AirradiansTranscriptIDs, 
          file = "Output/Transcriptomics/WGCNA/challenge/Frontloading/UPmodulePINK_Frontloaded.csv")
...
#                              Df    Sum Sq       H p.value
# pCO2_history                  1   3647281 14.1214 0.00017
# MatchMismatch                 1    106798  0.4135 0.52020
# pCO2_history:MatchMismatch    1   8027372 31.0801 0.00000
...
Frontload_modDunn <-  dunnTest(mean.vstExp ~ IDvar,
              ...
              method="bh")
# Comparison
# lowbaseline - lowchange	-4.396784	1.098668e-05	3.296004e-05	*
# lowbaseline - severebaseline	-6.599288	4.131375e-11	2.478825e-10 *	
# lowchange - severebaseline	-2.202504	2.762971e-02	3.315565e-02	
# lowbaseline - severechange	-3.111895	1.858903e-03	2.788355e-03	*
# lowchange - severechange	1.284888	1.988314e-01	1.988314e-01	
# severebaseline - severechange	3.487392	4.877551e-04	9.755103e-04	*	
...
# Out put the plots 
pdf("Output/Transcriptomics/WGCNA/challenge/Frontloading/UPmodulePINK_Frontloading.pdf", height=4, width =8)
...
Activated_AirradiansTranscriptIDs <- merge((df_total_1.1 %>% filter(Frontloaded_criteria == 'activated')),
                                             AirrdiansTranscript_Proteinnames_2, 
                                             by = c('Protein_name','Airradians.TranscriptID'))
nrow(Activated_AirradiansTranscriptIDs) # 56


# write csv of dfrontloaded genes
write.csv(Activated_AirradiansTranscriptIDs, 
          file = "Output/Transcriptomics/WGCNA/challenge/Frontloading/UPmodulePINK_Activated.csv")
...
pdf("Output/Transcriptomics/WGCNA/challenge/Frontloading/UPmodulePINK_FrontloadingAcivated.pdf", height=8, width =8)
ggarrange(Frontloaded_plot_1.1, Frontloaded_MEAN.SE_1.1,
...
write.csv(CostlyActivation_AirradiansTranscriptIDs, 
          file = "Output/Transcriptomics/WGCNA/challenge/Frontloading/DOWNmoduleBROWN_CostlyActivation.csv")
# truncate the vst Exp means (mean exp for each gene within pCO2 history * pCO2 exposure, N = 5 each)
...
# pCO2_history                  1    6.4   6.351  21.270 4.19e-06 ***
# MatchMismatch                 1    0.9   0.907   3.039  0.08140 .  
# pCO2_history:MatchMismatch    1    2.1   2.148   7.194  0.00736 ** 
# Residuals                  2508  748.8   0.299  
mod <- scheirerRayHare(mean.vstExp~pCO2_history*MatchMismatch,data=CostlyActivation_vst_levels)
#                              Df     Sum Sq      H   p.value
# pCO2_history                  1   21175543 40.254 0.0000000
# MatchMismatch                 1    3415879  6.493 0.0108276
# pCO2_history:MatchMismatch    1    9072984 17.247 0.0000328
# Residuals                  2508 1287258852
...
modDunn <-  dunnTest(mean.vstExp ~ IDvar,
              ...
              method="bh")
# lowbaseline - lowchange	1.134740	2.564842e-01	2.564842e-01	
# lowbaseline - severebaseline	7.422884	1.145973e-13	6.875835e-13	*
# lowchange - severebaseline	6.288144	3.212843e-10	9.638528e-10	 *
# lowbaseline - severechange	2.684426	7.265448e-03	1.089817e-02	
# lowchange - severechange	1.549686	1.212169e-01	1.454603e-01	
# severebaseline - severechange	-4.738458	2.153507e-06	4.307014e-06 *
...
CostlyActivation_MEAN.SE_1.2 <- CostlyActivation_plotting %>% 
        dplyr::filter(MatchMismatch %in% c('baseline', 'change')) %>% 
             ggplot(aes(x=MatchMismatch, y = mean_meanvstExp, fill=pCO2_history)) +
               ...
               labs(y= "vst expression", 
                    x = "pCO2 exposure",
                    title = "Costly activation genes (N=628)") + 
               geom_errorbar(aes(ymin=(mean_meanvstExp)-(se_vstExp), # new means and se by treatment
                                 ymax=(mean_meanvstExp)+(se_vstExp)), # new means and se by treatment
                                 width=0,position=position_dodge(.4)) # width determines the length of the end ticks
...
# Out put the plots 
pdf("Output/Transcriptomics/WGCNA/challenge/Frontloading/DOWNmoduleBROWN_CostlyActivation.pdf", height=4, width =8)
