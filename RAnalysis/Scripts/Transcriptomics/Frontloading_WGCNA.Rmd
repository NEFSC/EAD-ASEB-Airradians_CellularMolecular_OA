...
for (i in 1:nrow(loop_df)) {
  ...
  loop.table$baseMeanNAIVE_control       <- (df_loop %>% dplyr::filter(pCO2_exposure %in% 'severe' & # severe under severe - they are naive to low
                                                                         pCO2_history %in% 'severe'))$mean.vstExp
  loop.table$baseMeanHABITUATED_control  <- (df_loop %>% dplyr::filter(pCO2_exposure %in% 'severe' &  # low under severe - the habiturated control - low's response to severe, must be > severexsevere
                                                                         pCO2_history %in% 'low'))$mean.vstExp
  
  loop.table$baseMeanNAIVE_response      <- (df_loop %>% dplyr::filter(pCO2_exposure %in% 'low' & #  severe's response to low 
                                                                         pCO2_history %in% 'severe'))$mean.vstExp
  loop.table$baseMeanHABITUATED_response <- (df_loop %>% dplyr::filter(pCO2_exposure %in% 'low' &  # lows response to low
                                                                         pCO2_history %in% 'low'))$mean.vstExp
...
IDtargets_stats <- IDtargets %>% 
                      ...
                      dplyr::mutate(MatchMismatch = case_when(
                        (pCO2_history %in% 'severe'  & pCO2_exposure %in% 'severe')    ~ "match",
                        (pCO2_history %in% 'low' & pCO2_exposure %in% 'low') ~ "match",
                        (pCO2_history %in% 'severe'  & pCO2_exposure %in% 'low') ~ "mismatch",
                        (pCO2_history %in% 'low' & pCO2_exposure %in% 'severe')    ~ "mismatch",TRUE ~ NA
                      ))
IDtargets_stats$MatchMismatch <- as.factor(IDtargets_stats$MatchMismatch)
mod <- lm(mean.vstExp~pCO2_history*MatchMismatch,data=IDtargets_stats)
shapiro.test(resid(mod)) # p-value < 2.2e-16
...
mod <- scheirerRayHare(mean.vstExp~pCO2_history*MatchMismatch,data=IDtargets_stats)
#                              Df    Sum Sq      H p.value
# pCO2_history                  1   2006860 7.4618 0.00630
# MatchMismatch                 1    624115 2.3206 0.12767
# pCO2_history:MatchMismatch    1     11915 0.0443 0.83329
# Residuals                  1792 480124154 
...
pdf("Output/Transcriptomics/WGCNA/challenge/Frontloading/DOWNgreenpink_LOW.pdf", height=4, width =8)
