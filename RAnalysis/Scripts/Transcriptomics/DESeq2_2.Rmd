...
pairs(vstExp.filt_cohort.PLOTTING[c(2,3,4,5,7,8,9,10)], 
...
pairs(vstExp.filt_cohort.PLOTTING[c(7,8,9,10,12,13,14,15)], 
                          upper.panel = reg, # replace HERE for panel.smooth #
                          cex = 1.5, pch = 19, 
                          col = adjustcolor(4, .4), 
                          cex.labels = 2, 
...
pairs(vstExp.filt_cohort.PLOTTING[c(2,3,4,5,12,13,14,15)], 
                          upper.panel = reg, # replace HERE for panel.smooth #
                          cex = 1.5, pch = 19, 
                          col = adjustcolor(4, .4), 
                          cex.labels = 2, 
...
LowpCO2_cohort_Down <- LowpCO2_cohort_Down[duplicated(LowpCO2_cohort_Down$Airradians_TranscriptID),]
...
ModpCO2_cohort_Up   <- rbind(ModpCO2_cohort_Up_1, ModpCO2_cohort_Up_2) %>% dplyr::arrange(log2FoldChange)
  
ModpCO2_cohort_Up <- ModpCO2_cohort_Up[duplicated(ModpCO2_cohort_Up$Airradians_TranscriptID),]
...
HighpCO2_cohort_DOWN <- Master_DEGs %>% dplyr::filter(About %in% c('Low_v_High','Moderate_v_High') & 
                                                          Dir %in% 'up') %>% 
                       dplyr::arrange(Airradians_TranscriptID)
HighpCO2_cohort_DOWN <- HighpCO2_cohort_DOWN[duplicated(HighpCO2_cohort_DOWN$Airradians_TranscriptID),]
...
ModpCO2_cohort_Down   <- rbind(ModpCO2_cohort_Down_1, ModpCO2_cohort_Down_2) %>% dplyr::arrange(log2FoldChange)
  
ModpCO2_cohort_Down <- ModpCO2_cohort_Down[duplicated(ModpCO2_cohort_Down$Airradians_TranscriptID),]
write.csv(HighpCO2_cohort_DOWN, "Output/Transcriptomics/DESeq2/Venn_intersections/Low_expression_ModeratepCO2.csv")
...
HighpCO2_cohort_UP <- Master_DEGs %>% dplyr::filter(About %in% c('Low_v_High','Moderate_v_High') & 
                                                          Dir %in% 'down') %>% 
                       dplyr::arrange(Airradians_TranscriptID)
HighpCO2_cohort_UP <- HighpCO2_cohort_UP[duplicated(HighpCO2_cohort_UP$Airradians_TranscriptID),]
