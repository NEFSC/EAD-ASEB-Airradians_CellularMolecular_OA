...
Frontloaded_df  <- read.csv("Output/Transcriptomics/WGCNA/challenge/WGCNA_Frontloaded_transcripts_moduleredsalmon.csv")
...
```{r GO.terms_data}
# call the GO terms
Cvirginica_GOterms                <- as.data.frame(Frontloaded_df) %>% dplyr::select(c('Airradians.TranscriptID','Annotation_GO_ID'))
...
GO_unique.genes.all  <- as.vector(unique(Frontloaded_df$Airradians.TranscriptID)) # call all unique genes for GO analysis (goseq)

# Gene length 
GO_gene.length  <- unique(Frontloaded_df %>% dplyr::select(c("Airradians.TranscriptID","Cvirginica_length")))

# merge length with counts data
length_vector           <- GO_gene.length$Cvirginica_length

GeneLength.d2           <-  dplyr::inner_join(Frontloaded_df, 
                                              unique((GO_gene.length %>% 
...
sum(sapply(length_vector.d2,length)) == dim(Frontloaded_df)[1] #should be TRUE
...
Mod_integer <- as.integer(GO_unique.genes.all %in% (Frontloaded_df$Airradians.TranscriptID)) # w/o day-specific ID vector
