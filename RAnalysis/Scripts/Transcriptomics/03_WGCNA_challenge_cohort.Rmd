...
## NOTE - NEW ANALYSIS WITH NEW GENOME MAPPING - completed to the load checkpoint chunk on 11/1/2024
### Set working dir
...
# gather the 5CPM filtered read matrices 
...
nrow(filtered_counts_cohort) # 12163 subset from the 5CPM cutoff
...
dim(dds_1raw.vst.transposed) # 30 49900
dim(dds_1filt.vst.transposed) # 30 9866

dim(dds_2raw.vst.transposed) # 15 49900
dim(dds_2filt.vst.transposed) # 15 12163
...
abline(h = 32, col = "red") # cutoff of 32 - visual with abline here to see if this does the trick!
...
abline(h = 24, col = "red") # cutoff of 40 - visual with abline here to see if this does the trick!
...
# (1) Response to challenge 
clust_filt1 = cutreeStatic(sampleTree_filt1, cutHeight = 32, minSize = 10) # Determine cluster under the line
...
nGenes   = ncol(dds_1filt_master) # number of genes == 9866 
...
dim(dds_1filt_master) # 27 9866 - transformed count data now  has 27 samples - 3 omitted
...
# (2) Cohort differences under *match*
clust_filt2 = cutreeStatic(sampleTree_filt2, cutHeight = 24, minSize = 10) # Determine cluster under the line
...
nGenes   = ncol(dds_2filt_master) # number of genes == 12163 
...
dim(dds_2filt_master) # 14 12163 - transformed count data now  has 27 samples - 3 omitted
...
dev.off() # output 
# SUMMARY choose soft threshold of 8
...
dev.off() # output 
# SUMMARY choose soft threshold of 10 
```
...
```{r adjacency matrix}
# (1) Response to challenge 
softPower_1      = 8 # set your soft threshold based on the plots above 
...
# 2229 1763 1203  660  657  619  564  530  511  489  450  191 
...
# 1536 1085  997  972  895  819  803  727  706  597  562  534  468  398  304  277  259  224
```
...
# dynamicColors_sign_1
      # black        blue       brown       green greenyellow     magenta        pink      purple         red         tan   turquoise      yellow 
      #   564        1763        1203         657         450         511         530         489         619         191        2229         660 
sizeGrWindow(8,6)
...
# dynamicColors_sign_2
       # black         blue        brown         cyan        green  greenyellow       grey60    lightcyan   lightgreen      magenta midnightblue 
       #   803         1085          997          398          895          562          259          277          224          706          304 
       #  pink       purple          red       salmon          tan    turquoise       yellow 
       #   727          597          819          468          534         1536          972 
sizeGrWindow(8,6)
...
**Note** connectivity of the cohorts eigenges resulted in redundant module-eignenge correlations 
Feel free to run everything through to the heatmap stage to see what I mean here, inpaticular 
modules brown and black have thesame correlation and are clustered together! 
modules tan and grey60 also have the same correlation BUT are not clustered together. 
**Objectve** use 'MEDissThres' to cut this tree consolidating together redundant modules, our main target is 
modules brown and blank however this wil likely merge more modules as well. 

* view the tree from the previous chunk, what heigh can we assign as 'MEDissThres' to merge brown and black?
  - appears 0.4 should do it, run a few and plot to visualize the outcome of different heights! 
```{r} 

# assign cut herigheignengene tree! 
MEDissThres_2 = 0.4 # call the cut height
# Plot the cut line into the dendrogram
plot(METree_2, main = "Clustering of module eigengenes (dissimilarity calc = MEDiss = 1-cor(MEs))",
     xlab = "", sub = "") +
  abline(h=MEDissThres_2, col = "red") # add the line to the tree
...
# 'Merge' Specify the cut line for the dendrogram (module) - Calc MODULE EIGENGENES (MEs_2_merge)
...
sizeGrWindow(7, 6)
png("Output/Transcriptomics/WGCNA/cohorts/Cohorts_ClusterEigengenes_merge.png", 1000, 1000, pointsize=20)
plot(METree_2_merge, main = "Clustering of module eigengenes - SIGNED (dissimilarity calc = MEDiss = 1-cor(MEs))",
     xlab = "", sub = "")
dev.off()
```
...
write.csv(MEs_2_merge, file = "Output/Transcriptomics/WGCNA/cohorts/Cohorts_ModulEigengenes_merge.csv") # save the module eigengenes
```
...
MMPvalue_1             = as.data.frame(corPvalueStudent(as.matrix(geneModuleMembership_1), nSamples_1));
...
lowlow_GSPvalue = as.data.frame(corPvalueStudent(as.matrix(lowlow_geneTraitSignificance), nSamples_1));
...
MMPvalue_2             = as.data.frame(corPvalueStudent(as.matrix(geneModuleMembership_2), nSamples_2));
...
low_GSPvalue = as.data.frame(corPvalueStudent(as.matrix(low_geneTraitSignificance), nSamples_2));
...
```{r}
# (1) Response to challenge 
length(colnames(dds_1filt_master)[moduleColors_1=="blue"]) # 1763 total genes in the blue module
length(colnames(dds_1filt_master)[moduleColors_1=="red"]) # 619 total genes in the red module


# (2) Cohort differences under *match*
length(colnames(dds_2filt_master)[moduleColors_2=="blue"]) # 1085 total genes in the blue module
length(colnames(dds_2filt_master)[moduleColors_2=="red"]) # 819 total genes in the red module
...
# [1] "sseqid"                       "NCBI_refseq"                  "transcript_id"                "gene_id"                     
# [5] "blastxEval_CvirgTranscriptID" "blastxEval_CvirgProteinID"    "blastxEval_CvirgGeneID"       "meanLength"                  
# [9] "blastxEval_CvirgGOterms"  
probes1       = names(as.data.frame(dds_1filt_master))
probes1annot = match(probes1, annot$transcript_id)

probes2       = names(as.data.frame(dds_2filt_master))
probes2annot = match(probes2, annot$transcript_id)

# The following is the number or probes without annotation:

sum(is.na(probes1annot)) # 1589
length(probes1annot) # 9866 - 1589 = xxx of the filtered gene set has annotation via this Cvirg blast method
((9866-1589)/9866)*100 # 83.89418 % of genes annotated

sum(is.na(probes2annot)) # 1990
length(probes2annot) # 12163 - 1990 = xxx of the filtered gene set has annotation via this Cvirg blast method
((12163-1990)/12163)*100 # 83.6389 % of genes annotated
...
Airr_Cvirg_Cgig_KEGG <- merge(Cvirg_KEGGIDs_Ref, Airr_Cvirg_diamond, by = 'Cvirginica_TranscriptID') %>% 
                          dplyr::rename(Airradians.TranscriptID = sseqid,
...
geneInfo_GROUPS_1 = data.frame(Airradians.TranscriptID = annot$sseqid[probes1annot],
...
length(probes2annot)
names(low_GSPvalue)
geneInfo_GROUPS_2 = data.frame(Airradians.TranscriptID = annot$sseqid[probes2annot],
...
dim(dds_2filt_rlogtrans) # 12163   15 - note there are 15 samples here, not ommitted from when run WGCNA
