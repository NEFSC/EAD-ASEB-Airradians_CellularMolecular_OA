...
filtered_counts <-
                  read.csv(file="Output/Transcriptomics/Filtered_count_matrix/filtered_count_matrix_3cpm33perc.csv", 
                                                               sep=',', 
                                                               header=TRUE)  %>% 
                      dplyr::rename(transcript_id = X) %>% 
                      tibble::column_to_rownames(var = "transcript_id") %>% 
...
nrow(filtered_counts) # 9492 subset from the 3CPM cutoff
```
...
raw_counts_1      <- raw_counts[,exp_metadata_1$Sample_num]
filtered_counts_1 <- filtered_counts[,exp_metadata_1$Sample_num]
```
...
filtered_counts_2 <- filtered_counts[,exp_metadata_2$Sample_num]
```
...
dim(dds_1filt.vst.transposed) # 30 9492

dim(dds_2raw.vst.transposed) # 15 26686
dim(dds_2filt.vst.transposed) # 15 9492
...
abline(h = 40, col = "red") # cutoff of 40 - visual with abline here to see if this does the trick!
...
dim(dds_1filt_master) # 27 9492 - transformed count data now  has 27 samples - 3 omitted
...
# (2) Cohort differences under *match*
clust_filt2 = cutreeStatic(sampleTree_filt2, cutHeight = 40, minSize = 10) # Determine cluster under the line
...
dim(dds_2filt_master) # 14 9492 - transformed count data now  has 27 samples - 3 omitted
...
# 1611 1518  971  823  785  558  516  454  440  431  398  398  373  216  
...
#    2 1651 1159 1050 1000  901  802  674  665  646  566  376 
```
...
```{r}
# (1) Response to challenge 
length(colnames(dds_1filt_master)[moduleColors_1=="blue"]) # 1518 total genes in the blue module
length(colnames(dds_1filt_master)[moduleColors_1=="red"]) # 558 total genes in the red module


# (2) Cohort differences under *match*
length(colnames(dds_2filt_master)[moduleColors_2=="blue"]) # 1159 total genes in the blue module
length(colnames(dds_2filt_master)[moduleColors_2=="red"]) # 802 total genes in the red module
...
probes       = names(as.data.frame(dds_1filt_master))
probes2annot = match(probes, annot$Airradians_TranscriptID)
# The following is the number or probes without annotation:
sum(is.na(probes2annot)) # 1493
length(probes2annot) # 9492 - 1493 = 7999 of the filtered gene set has annotation via this Cvirg blast method
...
for(i in 1:nrow(modcolor)) {
  ...
  rlog_Mod        <- dds_1filt_rlog_master %>% dplyr::filter(Airradians.TranscriptID %in% Mod_geneIDs[,1])
  ...
  merged_Expdata_Mod <- merge(rlog_Mod_MELT, exp_metadata_1_master, by ='Sample_num')
  
  # mean Exp response table 
  meanEXp_Mod <- merged_Expdata_Mod %>% 
    select(c('Sample_num','rlog_Expression','pCO2_history', 'pCO2_exposure', 'All_pCO2')) %>% 
  ...
  pCO2exposure.rlog.Mod <- meanEXp_Summary.pCO2exposure %>% 
  ...
  AllpCO2.rlog.Mod <- meanEXp_Summary.All.pCO2 %>% 
  ...
  #library(ggpubr)
  single.factor.plot <-  ggarrange( ggarrange(pCO2history.rlog.Mod, pCO2exposure.rlog.Mod, ncol=2),
                                    AllpCO2.rlog.Mod,      
                                    plotlist = NULL,
                                    ncol = 1,
                                    nrow = 2,
...
for(i in 1:nrow(modcolor)) {
  ...
  # mean Exp response table 
  meanEXp_Mod <- merged_Expdata_Mod %>% 
    select(c('Sample_num','rlog_Expression','pCO2_history')) %>% 
