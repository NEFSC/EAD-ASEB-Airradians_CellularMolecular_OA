...
FrontloadedUP_WGCNA.data <- read.csv(file="Output/Transcriptomics/WGCNA/challenge/Frontloading/UPmoduleredsalmon_Severe.csv",
                                   sep=',', header=TRUE) %>%  dplyr::rename('Airradians_TranscriptID' =
                                                                                   'Airradians.TranscriptID')

FrontloadedDOWN_WGCNA.data <- read.csv(file="Output/Transcriptomics/WGCNA/challenge/Frontloading/DOWNmodulegreenpink_Moderate.csv",
                                   sep=',', header=TRUE) %>%  dplyr::rename('Airradians_TranscriptID' =
                                                                                   'Airradians.TranscriptID')
...
# challenge 
nrow(Challenge_WGCNA.data) # 8113
nrow(Challenge_WGCNA.data %>%  na.omit()) # 4944
Challenge_WGCNA.data.merged <- merge(Challenge_WGCNA.data, Airr_Cvirg_Cgig_KEGG, by = 'Airradians_TranscriptID')
nrow(Challenge_WGCNA.data.merged) # 6896 - note that many Airradians IDs do not have a corresponding Cvirg hit for annot
...
# Challenge
WGCNA_sigmodules_1 <- as.data.frame(c('red', 
                                      'salmon', 
                                      'pink', 
                                      'green'))

# Challenge - frontloaded genes
WGCNA_sigmodules_1.frontUP <- as.data.frame(c('red', 
                                          'salmon'))
...
Modgreen  <- as.data.frame(Challenge_WGCNA.data.merged %>% 
                                  dplyr::filter(moduleColor %in% 'green')  %>% 
                                  dplyr::select(c('Airradians_TranscriptID', 
                                                  'Cgigas_KEGGID', #'geneSymbol',
                                                   'p.MM.red')) %>% 
                                  dplyr::filter(.[[3]] < 0.05) %>%  # & .[[4]] > 0.6) %>% 
                                  ...
                                  dplyr::mutate(Cgigas_KEGGID = gsub(".*:","",Cgigas_KEGGID)) %>% 
                                  unnest(Cgigas_KEGGID))
Modpink  <- as.data.frame(Challenge_WGCNA.data.merged %>% 
                                  dplyr::filter(moduleColor %in% 'pink')  %>% 
                                  dplyr::select(c('Airradians_TranscriptID', 
                                                  'Cgigas_KEGGID', #'geneSymbol',
                                                   'p.MM.salmon')) %>% 
                                  dplyr::filter(.[[3]] < 0.05) %>%  # & .[[4]] > 0.6) %>% 
                                  ...
                                  dplyr::mutate(Cgigas_KEGGID = gsub(".*:","",Cgigas_KEGGID)) %>% 
                                  unnest(Cgigas_KEGGID))
Modules_greenpink <- rbind(Modgreen[,c(1:2)],Modpink[,c(1:2)])
nrow(Modules_greenpink) # 179

Modules_greenpink.frontloaded <- Modules_greenpink %>% 
                                  dplyr::filter(Airradians_TranscriptID %in% 
                                                FrontloadedDOWN_WGCNA.data$Airradians_TranscriptID)
nrow(Modules_greenpink.frontloaded) # 68
entrezID_vector <- as.vector(as.numeric(Modules_greenpink.frontloaded$Cgigas_KEGGID))
KEGG_cgigas     <- enrichKEGG(gene = entrezID_vector, 
                              organism  = 'crg', # 'hsa' is human 'crg' is pacific oyster 
                              pvalueCutoff = 0.05) 
df              <- as.data.frame(head(KEGG_cgigas))
